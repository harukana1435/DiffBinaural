...
# .npyファイルのパス
file_path = "/home/h-okano/DiffBinaural/processed_data/generated_mel_pos_right/000214.npy"
...
audio_input, sr = librosa.load("/home/h-okano/DiffBinaural/results_pos/result_000214/input_binaural.wav", mono=False, sr=None)
audio_sepstereo, sr = librosa.load("/home/h-okano/SepStereo/results/000214/predicted_binaural.wav", mono=False, sr=None)
audio_mono2binaural, sr = librosa.load("/home/h-okano/mono2binaural/results2/result_000214/predicted_binaural.wav", mono=False, sr=None)
...
input_mel = mel_spectrogram(torch.FloatTensor(input_process).unsqueeze(0), 1024, 80, 16000, 256, 1024, 0, 8000).squeeze(0).numpy()
sepstereo_mel = mel_spectrogram(torch.FloatTensor(sepstereo_process).unsqueeze(0), 1024, 80, 16000, 256, 1024, 0, 8000).squeeze(0).numpy()
mono2binaural_mel = mel_spectrogram(torch.FloatTensor(mono2binaural_process).unsqueeze(0), 1024, 80, 16000, 256, 1024, 0, 8000).squeeze(0).numpy()
mono_mel = mel_spectrogram(torch.FloatTensor(mono_process).unsqueeze(0), 1024, 80, 16000, 256, 1024, 0, 8000).squeeze(0).numpy()
