...
from melspec_process.melnet import MelNet
...
class tsv_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if wav.shape[0] != 1: # stereo to mono  (2,wav_len) -> (1,wav_len)
            wav_mean = ((wav[0]+wav[1])/2).unsqueeze(0)
            wav_diff = ((wav[0]-wav[1])/2).unsqueeze(0)
        else:
            wav_mean = wav
            wav_diff = wav
        wav_mean = torchaudio.functional.resample(wav_mean, orig_freq=orisr, new_freq=self.sr)
        wav_diff = torchaudio.functional.resample(wav_diff, orig_freq=orisr, new_freq=self.sr)

        if self.mode == 'pad':
            assert self.target_mel_length is not None
            wav_mean = self.pad_wav(wav_mean)
            wav_diff = self.pad_wav(wav_diff)
        return audio_path,wav_mean, wav_diff
...
def process_audio_by_tsv(rank,args):
    ...
    for batch in loader:
        audio_paths,wavs_mean, wavs_diff = batch
        wavs_mean = wavs_mean.to(device)
        wavs_diff = wavs_diff.to(device)
        
        if args.save_resample:               
            for audio_path,wav_mean, wav_diff in zip(audio_paths,wavs_mean, wavs_diff):
                ...
                #np.save(resample_path,wav_mean.cpu().numpy().squeeze(0))  
        ...
        if args.save_mel:
            ...
            for audio_path,wav_mean, wav_diff in zip(audio_paths,wavs_mean, wavs_diff):
                psplits = audio_path.split('/')
                wav_name = psplits[-1]
                mel_root,mel_mean_name = root,wav_name[:-4]+'_mel_mean.npy'
                mel_diff_name = wav_name[:-4]+'_mel_diff.npy'
                mel_dir_name_mean = os.path.join(mel_root,'melspec_mean')
                mel_dir_name_diff = os.path.join(mel_root,'melspec_diff')
                mel_mean_path = os.path.join(mel_dir_name_mean,mel_mean_name)
                mel_diff_path = os.path.join(mel_dir_name_diff,mel_diff_name)
                # スペクトログラムの和の方
                if not os.path.exists(mel_mean_path):
                    mel_spec = mel_net(wav_mean).cpu().numpy().squeeze(0) # (mel_bins,mel_len) 
                    ...
                    os.makedirs(mel_dir_name_mean,exist_ok=True)
                    np.save(mel_mean_path,mel_spec) 
                # スペクトログラムの差の方
                if not os.path.exists(mel_diff_path):
                    mel_spec = mel_net(wav_diff).cpu().numpy().squeeze(0) # (mel_bins,mel_len) 
                    if mel_spec.shape[1] <= batch_max_length:
                        if mode == 'tile': # pad is done in dataset as pad wav
                            n_repeat = math.ceil((batch_max_length + 1) / mel_spec.shape[1])
                            mel_spec = np.tile(mel_spec,reps=(1,n_repeat))
                    ...
                    mel_spec = mel_spec[:,:batch_max_length]
                    #print(mel_spec.shape)
                    os.makedirs(mel_dir_name_diff,exist_ok=True)
                    np.save(mel_diff_path,mel_spec)          
...
def parse_args():
    ...
    parser.add_argument( "--tsv_path",type=str, default="/home/h-okano/DiffBinaural/melspec_process/data/output.tsv")
